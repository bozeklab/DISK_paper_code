...
if __name__ == '__main__':
    ...
    mbi = pd.read_csv(
        os.path.join(basedir, f'results_behavior/MarkerBasedImputation_FL2/model_ensemble/test_repeat-0_merged/test_w-all-nans_file{i_file}_merged/test_w-all-nans_file{i_file}_sample0_MBI.csv'))
    n_methods = 4
    orig = pd.read_csv(
    ...
    for j in range(3):
        axes[0, j].plot(t, x[:, i_kp * 3 + j], 'o-', ms=1)
        axes[1, j].plot(t, x[:, i_kp * 3 + j], 'o-', ms=1)
        axes[2, j].plot(t, x[:, i_kp * 3 + j], 'o-', ms=1)
        axes[2, j].plot(t, x[:, i_kp * 3 + j], 'o-', ms=1)
        ...
        for b, e in zip(begins, ends):
            axes[0, j].axvspan(xmin=t[b], xmax=t[e + 1], alpha=0.2)
            axes[1, j].axvspan(xmin=t[b], xmax=t[e + 1], alpha=0.2)
            axes[2, j].axvspan(xmin=t[b], xmax=t[e + 1], alpha=0.2)
            axes[3, j].axvspan(xmin=t[b], xmax=t[e + 1], alpha=0.2)
    ...
    axes[0, 1].set_title('Y')
    axes[0, 2].set_title('Z')
    plt.savefig(os.path.join(output_folder, 'test_w-all-nans_file7_DISK_holes_shaded.png'))
    ...
    axes[0, 1].set_title('Y')
    axes[0, 2].set_title('Z')
    plt.savefig(os.path.join(output_folder, f'test_w-all-nans_file{i_file}_model_10_5_1_holes_shaded.png'))
