...
def load_keypoints(
    ...
    if format == "deeplabcut":
        ...
        """elif format == "disk":
        if type(filepath_pattern) == str:
            if os.path.isdir(filepath_pattern):
                dataset_constant_file = glob(os.path.join(filepath_pattern, 'constants.py'))[0]
            else:
                print(os.path.join(os.path.dirname(filepath_pattern), 'constants.py'), glob(os.path.join(os.path.dirname(filepath_pattern), 'constants.py')))
                dataset_constant_file = glob(os.path.join(os.path.dirname(filepath_pattern), 'constants.py'))[0]
        
            additional_args = {'dataset_constants': read_constant_file(dataset_constant_file)}
        else:
            dataset_constant_file = [glob(os.path.join(os.path.dirname(f), 'constants.py'))[0] for f in filepath_pattern]
            additional_args = {'dataset_constants': [read_constant_file(f) for f in dataset_constant_file]}"""
    else:
        additional_args = {}
    ...
    if format == "disk": 
        if os.path.isdir(filepaths[0]):
            dataset_constant_file = glob(os.path.join(filepath_pattern[0], 'constants.py'))[0]
        else:
            dataset_constant_file = glob(os.path.join(os.path.dirname(filepaths[0]), 'constants.py'))[0]
        additional_args = {'dataset_constants': read_constant_file(dataset_constant_file)}
...
def _disk_loader(filepath, name, dataset_constants):
    """Load keypoints from DISK already processed .npy files."""
